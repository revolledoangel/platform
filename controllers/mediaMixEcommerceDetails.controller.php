...
class MediaMixEcommerceDetails_Controller {
    static public function ctrGetMediaMixById($mmreId) {
        ...
        $sql = "SELECT d.*, p.name AS project_name, p.code AS project_code, p.group AS project_group, p.active AS project_active,
                ...
                WHERE d.mediamixecommerce_id = $mmreId";
        $res = $conn->query($sql);
        if ($res) {
            while ($row = $res->fetch_assoc()) {
                ...
                // Objectives
                $sqlO = "SELECT o.id, o.name, o.code FROM mme_details_objectives mo LEFT JOIN objectives o ON mo.objective_id = o.id WHERE mo.mme_detail_id = {$detail['id']}";
    ...
    static public function eliminarDetalle($id) {
        ...
        if ($conn->connect_error) return false;
        $id = intval($id);
        $sql = "DELETE FROM mediamixecommerce_details WHERE id = $id";
                ...
                if ($conn->query($sql)) {
                    echo '<script>
                        swal({
                            ...
                            window.location = "mediaMixEcommerceDetails?mediaMixId=' . $mediaMixId . '";
                        });
                    </script>';
                } else {
