<?php
class MediaMixRealEstate_Controller
{
    static public function ctrShowMediaMixRealEstate()
    {
        $url = 'https://algoritmo.digital/backend/public/api/mmres';
        $ch = curl_init($url);
    ...
    public function ctrCreateMediaMixRealEstate()
    {
        if (isset($_POST["newPeriodId"])) {
            if ($_POST["newName"] === "" || preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ \-.,()]+$/', $_POST["newName"])) {
                ...
                $ch = curl_init('https://algoritmo.digital/backend/public/api/mmres');
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                ...
                if ($httpCode === 201) {
                    echo '<script>
                            swal({
                                ...
                                if(result.value){ window.location = "mediaMixRealEstate"; }
                            });
                        </script>';
                } else {
    ...
    public function ctrEditMediaMixRealEstate()
    {
    ...
    static public function ctrDeleteMediaMixRealEstate()
    {
    ...
    static public function ctrShowMediaMixRealEstateById($id)
    {
        $url = 'https://algoritmo.digital/backend/public/api/mmres/' . $id;
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, ['Accept: application/json']);
        $response = curl_exec($ch);
