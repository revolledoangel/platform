...
class MonthlyFeedback_Controller
{
    ...
    static public function ctrSubmitFeedbackResponse()
    {
        ...
        if ($ok && !empty($webhookUrl)) {
            $feedbackRow['token'] = $token; // needed for PDF URL in email
    ...
    private static function postWebhook($feedbackRow, $responseId, $responseData, $webhookUrl)
    {
        ...
        // PDF download link (no login required)
        $pdfToken = $feedbackRow['token'] ?? '';
        if ($pdfToken && $responseId) {
            $pdfUrl = $baseUrl . '/platform/index.php?route=feedbackResponsePdf'
                    . '&token='      . urlencode($pdfToken)
                    . '&responseId=' . (int)$responseId;
            $h .= '<div style="margin-top:20px;padding:16px 20px;background:#f5f0ff;border:1px solid #d8b4fe;border-radius:8px;text-align:center;">';
            $h .= '<p style="margin:0 0 10px;font-size:13px;color:#555;">&#128196; Ver y descargar el reporte completo en PDF:</p>';
            $h .= '<a href="' . htmlspecialchars($pdfUrl) . '" style="display:inline-block;background:#4614FF;color:#fff;padding:10px 24px;border-radius:6px;text-decoration:none;font-weight:700;font-size:14px;">&#11015; Descargar PDF del reporte</a>';
            $h .= '<p style="margin:8px 0 0;font-size:11px;color:#999;">No requiere iniciar sesión</p>';
            $h .= '</div>';
        }
