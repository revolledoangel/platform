...
class AjaxMediaMixEcommerce
{
    public $mediaMixId;

    public function ajaxEditMediaMixEcommerce()
    {
        $url = "https://algoritmo.digital/backend/public/api/mmecommerces/" . $this->mediaMixId;
        $response = file_get_contents($url);
        echo $response;
    }

    public function ajaxListMediaMixEcommerce()
    {
        $records = MediaMixEcommerce_Controller::ctrShowMediaMixEcommerce();
        if (empty($records)) {
        ...
        foreach ($records as $key => $record) {
            $acciones = '<div class="btn-group">
                <a href="mediaMixEcommerceDetails?mediaMixId=' . $record["id"] . '" class="btn btn-info">
                    <i class="fa fa-eye"></i>
                </a>
                <button type="button" class="btn btn-warning btn-editMediaMix" mediaMixId="' . $record["id"] . '" data-toggle="modal" data-target="#editMediaMixEcommerceModal">
    ...
    public function ajaxCloneMediaMixEcommerce()
    {
        $url = "https://algoritmo.digital/backend/public/api/mmecommerces/" . $this->mediaMixId . "/clone";
    ...
    public function ajaxCloneMediaMix()
    {
        ...
        try {
            ...
            $sqlOriginal = "SELECT * FROM mediamixecommerces WHERE id = $source_mix_id";
            ...
            $sqlInsertMix = "INSERT INTO mediamixecommerces 
            ...
            $sqlDetails = "SELECT * FROM mediamixecommerce_details 
                          WHERE mediamixecommerce_id = $source_mix_id $aonCondition";
            ...
            if ($resultDetails && $resultDetails->num_rows > 0) {
                while ($detail = $resultDetails->fetch_assoc()) {
                    ...
                    // Insertar detalle
                    $sqlInsertDetail = "INSERT INTO mediamixecommerce_details 
                                       (mediamixecommerce_id, project_id, channel_id, campaign_type_id, 
                    ...
                    $sqlFormats = "SELECT format_id FROM mme_details_formats WHERE mme_detail_id = $old_detail_id";
                    $resultFormats = $conn->query($sqlFormats);
                    
                    if ($resultFormats && $resultFormats->num_rows > 0) {
                        while ($format = $resultFormats->fetch_assoc()) {
                            $sqlInsertFormat = "INSERT INTO mme_details_formats (mme_detail_id, format_id) 
                    ...
                    $sqlObjectives = "SELECT objective_id FROM mme_details_objectives WHERE mme_detail_id = $old_detail_id";
                    ...
                    if ($resultObjectives && $resultObjectives->num_rows > 0) {
                        while ($objective = $resultObjectives->fetch_assoc()) {
                            $sqlInsertObjective = "INSERT INTO mme_details_objectives (mme_detail_id, objective_id) 
...
if (isset($_GET["action"]) && $_GET["action"] === "list") {
    $listar = new AjaxMediaMixEcommerce();
    $listar->ajaxListMediaMixEcommerce();
    return;
}

// Acción: editar
if (isset($_GET["mediaMixId"])) {
    $editar = new AjaxMediaMixEcommerce();
    $editar->mediaMixId = $_GET["mediaMixId"];
    $editar->ajaxEditMediaMixEcommerce();
}

// Acción: clonar
if (isset($_POST["cloneMediaMixId"]) && isset($_POST["period_id"])) {
    $clonar = new AjaxMediaMixEcommerce();
    $clonar->mediaMixId = $_POST["cloneMediaMixId"];
    $clonar->ajaxCloneMediaMixEcommerce();
}

// Acción: obtener períodos
if (isset($_POST["get_periods"])) {
    $periodos = new AjaxMediaMixEcommerce();
...
if (isset($_POST["action"]) && $_POST["action"] === "cloneMix") {
    $clone = new AjaxMediaMixEcommerce();
